...
function renderAuthExperience() {
  ...
  if (authSignupComplete) {
    ...
    const signupMessage = confirmed
      ? "<p>Tu cuenta se ha creado y la sesión está abierta.</p>"
      : `<p>Hemos creado la cuenta de ${escapeHtml(authSignupComplete.email)}. Revisa tu bandeja de entrada y, si no lo ves, la carpeta de spam.</p><p>Abre el enlace de confirmación y vuelve aquí para iniciar sesión. Podrás elegir tu alias y completar la inscripción solo después de entrar.</p>`;
    return `<div class="user-shell"><section class="user-card auth-card auth-success-card"><p class="eyebrow">${confirmed ? "Cuenta preparada" : "Revisa tu correo"}</p><h2>${confirmed ? "Ya puedes empezar" : "Confirma tu cuenta"}</h2>${signupMessage}<div class="auth-success-mark" aria-hidden="true">✓</div><button class="primary" type="button" data-auth-continue>${confirmed ? "Continuar a mi porra" : "Ir a iniciar sesión"}</button></section></div>`;
  }

...
Si quieres incluir perfiles y predicciones de participantes en el build, puedes pasar un JSON opcional:

```powershell
$env:PYTHONPATH = "src"
python -m porra_mundial.build_data --out public/datos.json --predictions data/predictions_profiles.example.json
```

El formato esperado es una lista de perfiles con `email`, `alias`, `updated_at` y una lista `predictions` con `matchid`, `home_score` y `away_score`.

...
- UEFA es la fuente primaria de calendario, resultados y bombos oficiales; TheSportsDB es una fuente secundaria de identificación y contraste. Ningún marcador, clasificado o bombo se inventa.
...
```text
[UEFA fixtures/results + pots] ─┐
                                 ├─> [importador Champions] ─> [semilla histórica]
[TheSportsDB API (secondary)] ─┘                                 │
                                                                  v
                                                        [generador existente]
                                                                  │
                                                                  v
...
- **Importador Champions:** transforma el fixture oficial identificado, filtra los eventos por etapa incluida, normaliza los campos al contrato interno y conserva referencias UEFA y, si existe, un identificador de contraste TheSportsDB.
...
- `load_uefa_champions_fixture(season)` devuelve el fixture oficial versionado para la temporada solicitada; un fixture vacío o inválido es un error explícito.
- `build_historical_champions_seed(fixtures, pots, sportsdb_events)` devuelve una semilla con solo fase liga y eliminatorias, ordenada cronológicamente y con identificadores internos estables y referencias a sus fuentes.
...
1. Se descarga o carga un fixture oficial UEFA de la temporada 2025-26.
...
| --- | --- | --- |
| Metadatos y trazabilidad | Unitario | Fixture oficial UEFA + catálogo UEFA produce competición, temporada, URLs e identificadores de origen; la referencia TheSportsDB queda marcada como secundaria. |
...
| --- | --- | --- |
| TheSportsDB no cubre todos los eventos con la clave pública | La fuente secundaria no puede completar el histórico | El fixture oficial UEFA es autoritativo; la falta de correlación SportsDB se registra sin bloquear datos UEFA. |
...
| --- | --- | --- | --- | --- | --- |
| T001 |  | Add Champions season constants, UEFA pots catalogue, and official source fixtures | Targeted catalogue/normalisation tests pass and assert 36 clubs in four pots of nine | — | Spec §Goals; §Acceptance 1–3 |
...
| T003 |  | Implement UEFA historical Champions seed importer and generation command | T002 tests pass; generated seed contains only included rounds, UEFA metadata and optional TheSportsDB correlation | T002 | Spec §Behaviour; §Acceptance 1–3, 5 |
...
- Consumes: official UEFA season fixture, official UEFA pot catalogue and optional TheSportsDB season payload.
- Produces: reproducible seed with `meta` provenance, `bombos`, participants/demo data, normalized match list and optional source correlations; invalid UEFA source conditions are explicit.

...
from .predictions import build_prediction_payload, build_prediction_windows
from .scoring import build_datos_json
from .sheets import load_public_tsv_inputs, load_sheet_inputs
from .sportsdb import (
    fetch_champions_events_for_date,
    fetch_champions_events_for_round,
    parse_events,
    search_events,
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--predictions",
        default="",
        help="Ruta opcional a un JSON con perfiles y predicciones de participantes.",
    )
    args = parser.parse_args()

    inputs = _load_inputs()
    predictions_profiles = _load_predictions_profiles(args.predictions)
    if predictions_profiles is not None:
        inputs["predictions_profiles"] = predictions_profiles
    ...
    payload["meta"]["prediction_windows"] = build_prediction_windows(matches)
    payload["meta"]["prediction_payload"] = build_prediction_payload(
        inputs.get("predictions_profiles"),
        matches,
    )
    payload["meta"].update(
        {
...
def _load_predictions_profiles(path: str | None) -> list[dict[str, Any]] | None:
    if not path:
        return None
    candidate = Path(path)
    ...
    if isinstance(payload, dict):
        profiles = payload.get("profiles") or payload.get("participants") or payload.get("usuarios")
...
def _fetch_sportsdb_daily_matches(fetch_dates: list[str]) -> list[Match]:
    ...
    for fetch_date in fetch_dates:
        payload = fetch_champions_events_for_date(fetch_date)
...
def _fetch_sportsdb_knockout_matches() -> tuple[list[Match], list[str]]:
    ...
    for round_number in SPORTSDB_KNOCKOUT_ROUNDS:
        try:
            payload = fetch_champions_events_for_round(round_number)
        except Exception as exc:

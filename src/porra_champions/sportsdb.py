...
def fetch_champions_events(season: str = "2026") -> dict:
    ...
    """
    query = urlencode({"id": CHAMPIONS_LEAGUE_ID, "s": season})
    url = f"{BASE_URL}/eventsseason.php?{query}"
    with urlopen(url, timeout=30) as response:
        return json.loads(response.read().decode("utf-8"))


def fetch_champions_events_for_date(date: str) -> dict:
    """Descarga eventos de Champions League para un día concreto."""
    query = urlencode({"d": date, "l": CHAMPIONS_LEAGUE_ID})
    url = f"{BASE_URL}/eventsday.php?{query}"
    with urlopen(url, timeout=30) as response:
        return json.loads(response.read().decode("utf-8"))


def fetch_champions_events_for_round(round_number: int, season: str = "2026") -> dict:
    """Descarga todos los eventos de una ronda de Champions League."""
    query = urlencode({"id": CHAMPIONS_LEAGUE_ID, "r": round_number, "s": season})
